...
<?php foreach($models as $k=>$model):?>
    <div class="cat <?=$k==0?"item1":""?>">
        <h3>
            <?php //=\yii\helpers\Html::a($model->name,['index/list','cate_id'=>$model->id])?>
            <?=\yii\helpers\Html::a($model->name,['index/index'])?>
            <b></b>
        </h3>
        <div class="cat_detail">
            <?php
//            $parent=\backend\models\GoodsCategory::find()->where(['parent_id'=>$model->id])->all();?>
            <?php //foreach($parent as $k2=>$value):?>
            <?php foreach($model->children as $k2=>$value):?>
                <dl <?=$k2==0?'class="dl_1st"':''?>>
                    <dt><?=\yii\helpers\Html::a($value->name,['index/list','cate_id'=>$value->id])?></dt>
                    <?php
//                    $p=\backend\models\GoodsCategory::find()->where(['parent_id'=>$value->id])->all();
                    ?>
                    <?php //foreach($p as $v):?>
